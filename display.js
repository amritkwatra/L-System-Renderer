let translation;
let lsystem = new LSystem(350, 580);
...
function setup() {
    angleMode(DEGREES);
    var canvas = createCanvas(700, 580);
    canvas.parent("sketch_container")
    background(0);
    stroke(255, 255, 255);
    strokeWeight(1);
    lsystem.setLength(1.2);
...
function displayLsystemDetails(lsystem) {
    ...
    for (const i in lsystem.stroke_translations) {
        strokes_terms += lsystem.stroke_translations[i] + ", "
    }
    strokes_terms += "</p>"
